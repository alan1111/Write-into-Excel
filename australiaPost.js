...
var workbook = XLSX.readFile('trackingAustriliaPost.ods');
        ...
        request(defaultOption, function (error, response, body) {
            if(error) {
                ...
                if(response.statusCode !== 200) {
                    ...
                    let $ = body;
                    if (typeof $.QueryTrackEventsResponse.TrackingResults[0].Consignment.Articles[0] !== 'undefined') {
                        console.log('ok');
                        let datas = $.QueryTrackEventsResponse.TrackingResults[0].Consignment.Articles[0].Events;
                        if (datas) {
                            datas.forEach(data => {
                                items.push({
                                    trackingCode : trackingCode,
                                    dateTime : data.EventDateTime,
                                    status : data.EventDescription
                                });
                            });
                        }
                    } else {
...
// 导出 Excel
                    XLSX.writeFile(wb, 'trackingAP.xlsx');
                }
            }
        });
    });
...
new Promise(function (resolve, reject) {
    ...
    data.forEach(dat => {
        ...
        // return;
        getFromHtml({
            uri: 'https://digitalapi.auspost.com.au/track/v3/search?q='+trackingCode,
            headers: {
