...
var workbook = XLSX.readFile('trackingCP.ods');
        ...
        request(defaultOption, function (error, response, body) {
            if(error) {
                ...
                if(response.statusCode !== 200) {
                    ...
                    let $ = body;
                    if (typeof $.MainRootNode.Root[0] !== 'undefined') {
                        console.log('ok');
                        let datas = $.MainRootNode.Root[0].TrakingInfo;
                        datas.forEach(data => {
                            items.push({
...
// 导出 Excel
                    XLSX.writeFile(wb, 'trackingCP.xlsx');
                }
            }
        });
    });
...
new Promise(function (resolve, reject) {
    ...
    data.forEach(dat => {
        ...
        // return;
        getFromHtml({
            uri: 'https://www.couriersplease.com.au/DesktopModules/EzyTrack/EzyTrackHandler/CPPL_EzyTrackHandler.ashx?Type=TrakingJsonUser_v2',
            body: {
                'strClientCode': 'CPPLW',
                'strServiceCode': '123',
                'strCode': trackingCode
            }
        }, trackingCode);
    });
});
